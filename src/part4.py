...
<div class="codefile">
  ...
  <pre><span class="kw">type</span> PrimaryKeyStats <span class="kw">struct</span> {
    ...
    Min, Max int64                           <span class="cm">// 主键区间，用于范围剪枝</span>
...
<div class="codefile">
  ...
  <pre><span class="kw">type</span> PrimaryKeyStats <span class="kw">struct</span> {
    ...
    Min, Max int64                           <span class="cm">// PK range, for range pruning</span>
...
这种"<strong>宁可多查、绝不漏查</strong>"的非对称保证，恰恰是删除路由能放心剪枝的根基。Milvus 里这张布隆过滤器由 <span class="inline">FieldStats</span>（<span class="mono">internal/storage/field_stats.go</span>）持有——它装着字段的 min/max 与那张 <span class="inline">BF</span>（<span class="inline">bloomfilter.BloomFilterInterface</span>），是"删除该落到哪些段"这个问题的<strong>判定依据</strong>。</p>
...
<div class="codefile">
  <div class="cf-head"><span class="dot"></span><span class="path">internal/storage/field_stats.go</span><span class="ln">FieldStats：段级字段摘要，含布隆过滤器（节选）</span></div>
  <pre><span class="kw">type</span> FieldStats <span class="kw">struct</span> {
    FieldID int64                            <span class="cm">// 字段（含主键）</span>
    Max, Min ScalarFieldValue                <span class="cm">// 区间，用于范围剪枝</span>
    BF       bloomfilter.BloomFilterInterface <span class="cm">// 布隆：快速"查无此主键"，删除据此路由</span>
}</pre>
</div>
...
<div class="card macro">
  <div class="tag">🌍 宏观理解</div>
  一句话：在只追加的存储上，<strong>删除不是抹除而是盖一枚带时间戳的墓碑</strong>——它作为<strong>删除消息</strong>进 WAL、沉淀为 <strong>L0 deltalog</strong>，靠<strong>布隆过滤器</strong>（<span class="inline">FieldStats</span> 的 <span class="inline">BF</span>，<span class="mono">internal/storage/field_stats.go</span>）路由到可能含此主键的段，最终由 <strong>Level0DeleteCompaction</strong> 物理应用（第 19 课）。<strong>upsert = 删除 + 插入</strong>（同主键、更晚 ts 的新版本）。统一这一切的是<strong>按时间戳的 MVCC</strong>：以保证时间戳 T 读，一行可见 ⇔ 插入 ts ≤ T 且无 ts ≤ T 的墓碑作废它；T 怎么定由一致性级别决定（第 30 课）。
</div>
...
<div class="card key">
  ...
  <ul>
    ...
    <li><strong>布隆路由</strong>：拿待删主键逐段问布隆（<span class="inline">FieldStats.BF</span>，<span class="mono">internal/storage/field_stats.go</span>），"肯定没有"的段跳过，只把墓碑落到"可能含"的段；绝不漏报保证不错删。</li>
...
This asymmetric guarantee of "<strong>rather over-check than ever miss</strong>" is exactly the foundation on which delete routing can confidently prune. In Milvus this bloom filter is held by <span class="inline">FieldStats</span> (<span class="mono">internal/storage/field_stats.go</span>) — it carries the field's min/max and that <span class="inline">BF</span> (<span class="inline">bloomfilter.BloomFilterInterface</span>), the <strong>decision basis</strong> for "which segments a delete should land on."</p>
...
<div class="codefile">
  <div class="cf-head"><span class="dot"></span><span class="path">internal/storage/field_stats.go</span><span class="ln">FieldStats: a segment-level field summary, with bloom filter (excerpt)</span></div>
  <pre><span class="kw">type</span> FieldStats <span class="kw">struct</span> {
    FieldID int64                            <span class="cm">// the field (incl. PK)</span>
    Max, Min ScalarFieldValue                <span class="cm">// range, for range pruning</span>
    BF       bloomfilter.BloomFilterInterface <span class="cm">// bloom: quick "no such PK"; routes deletes</span>
}</pre>
</div>
...
<div class="card macro">
  <div class="tag">🌍 Big picture</div>
  In one line: on append-only storage, <strong>a delete is not an erase but stamping a timestamped tombstone</strong> — it enters the WAL as a <strong>delete message</strong>, settles into an <strong>L0 deltalog</strong>, is routed by <strong>bloom filters</strong> (<span class="inline">FieldStats</span>'s <span class="inline">BF</span>, <span class="mono">internal/storage/field_stats.go</span>) to segments that might hold the PK, and is finally physically applied by <strong>Level0DeleteCompaction</strong> (Lesson 19). <strong>upsert = delete + insert</strong> (same PK, a new version with later ts). Unifying it all is <strong>timestamp-based MVCC</strong>: read at guarantee-ts T, a row is visible ⇔ insert ts ≤ T and no tombstone with ts ≤ T voids it; how T is set is decided by the consistency level (Lesson 30).
</div>
...
<div class="card key">
  ...
  <ul>
    ...
    <li><strong>Bloom routing</strong>: ask each segment's bloom with the to-delete PK (<span class="inline">FieldStats.BF</span>, <span class="mono">internal/storage/field_stats.go</span>); "definitely not" segments are skipped, the tombstone lands only on "maybe" segments; no false negatives guarantees no wrong delete.</li>

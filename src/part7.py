...
<div class="fig">
  <svg viewBox="0 0 760 300" role="img" aria-label="拦截器链装配线：每次 Append 顺序穿过 Lock（最外层先上锁）、TimeTick、Txn、Shard 等拦截器，最后落到 WAL Backend，成为一条合规的不可变日志">
    ...
    <rect x="108" y="104" width="110" height="86" rx="9" style="fill:var(--panel);stroke:var(--amber);stroke-width:1.5"/><text x="163" y="140" text-anchor="middle" style="fill:var(--amber);font-weight:700">① Lock</text><text x="163" y="162" text-anchor="middle" style="fill:var(--muted)">最外层先上锁</text>
    <line x1="218" y1="147" x2="234" y2="147" style="stroke:var(--line);stroke-width:2"/><path d="M234,147 l-9,-4 l0,8 z" style="fill:var(--line)"/>
    <rect x="236" y="104" width="100" height="86" rx="9" style="fill:var(--accent-soft);stroke:var(--accent);stroke-width:1.5"/><text x="286" y="140" text-anchor="middle" style="fill:var(--accent-ink);font-weight:700">② TimeTick</text><text x="286" y="162" text-anchor="middle" style="fill:var(--muted)">盖单调戳</text>
    <line x1="336" y1="147" x2="352" y2="147" style="stroke:var(--line);stroke-width:2"/><path d="M352,147 l-9,-4 l0,8 z" style="fill:var(--line)"/>
    <rect x="354" y="104" width="100" height="86" rx="9" style="fill:var(--panel);stroke:var(--purple);stroke-width:1.5"/><text x="404" y="140" text-anchor="middle" style="fill:var(--purple);font-weight:700">③ Txn</text><text x="404" y="162" text-anchor="middle" style="fill:var(--muted)">事务原子</text>
    <line x1="454" y1="147" x2="470" y2="147" style="stroke:var(--line);stroke-width:2"/><path d="M470,147 l-9,-4 l0,8 z" style="fill:var(--line)"/>
    <rect x="472" y="104" width="100" height="86" rx="9" style="fill:var(--panel);stroke:var(--teal);stroke-width:1.5"/><text x="522" y="140" text-anchor="middle" style="fill:var(--teal);font-weight:700">④ Shard</text><text x="522" y="162" text-anchor="middle" style="fill:var(--muted)">段分配</text>
  ...
  <div class="figcap"><b>拦截器链 = 一条装配线</b>：每次 <span class="mono">Append</span> 顺序穿过 <b>① Lock</b>（最外层先上锁，持有到整段处理结束）<b>② TimeTick</b>（盖 PChannel 级单调戳，正是读路径 tsafe 等待的对象）<b>③ Txn</b>（事务原子性，由 TimeTick 拦截器内部处理）<b>④ Shard</b>（段分配，段 ID 此刻才定），最后 <b>⑤</b> 真正落到 WAL Backend。关注点正交、可插拔——要加新写语义，往链上挂一环即可，不动核心追加逻辑。</div>
</div>

<div class="vflow">
  <div class="step"><div class="num">1</div><div class="sc"><h4>Lock 拦截器（最外层）</h4><p>在 VChannel 或 PChannel 粒度上<strong>先获取锁并持有到整段处理结束</strong>，做<strong>独占/共享</strong>访问控制，保证并发写入的正确顺序——它在链的<strong>最外层</strong>。</p></div></div>
  <div class="step"><div class="num">2</div><div class="sc"><h4>TimeTick 拦截器</h4><p>给这条消息盖上 PChannel 级<strong>单调递增</strong>的 TimeTick——这把"日志时钟"正是读路径 tsafe 等待的对象（第 30 课）。</p></div></div>
  <div class="step"><div class="num">3</div><div class="sc"><h4>Txn（事务，由 TimeTick 拦截器内处理）</h4><p>管理<strong>事务</strong>的生命周期：把一批本应"要么都见、要么都不见"的写入，框在一个事务里原子地落日志。</p></div></div>
  <div class="step"><div class="num">4</div><div class="sc"><h4>Shard 拦截器</h4><p>做<strong>段分配</strong>：决定这批行落到哪个 growing 段（段 ID 在此刻才定，回忆第 15 课"段 ID 在 StreamingNode 才分配"）。</p></div></div>
...
<div class="card key">
  ...
  <ul>
    ...
    <li><strong>拦截器链</strong>：每次 Append 依次过 <strong>Lock / TimeTick / Txn / Shard</strong>（Lock 在最外层、先获取并持有），把上锁、盖时间戳、事务、段分配等横切关注点拆开处理。</li>
...
<div class="fig">
  <svg viewBox="0 0 760 300" role="img" aria-label="The interceptor chain as an assembly line: every Append threads Lock (outermost, acquired first), TimeTick, Txn and Shard in order, then lands in the WAL Backend as a compliant immutable log entry">
    ...
    <line x1="90" y1="147" x2="106" y2="147" style="stroke:var(--line);stroke-width:2"/><path d="M106,147 l-9,-4 l0,8 z" style="fill:var(--line)"/>
    <rect x="108" y="104" width="110" height="86" rx="9" style="fill:var(--panel);stroke:var(--amber);stroke-width:1.5"/><text x="163" y="140" text-anchor="middle" style="fill:var(--amber);font-weight:700">① Lock</text><text x="163" y="162" text-anchor="middle" style="fill:var(--muted)">lock (outer)</text>
    <line x1="218" y1="147" x2="234" y2="147" style="stroke:var(--line);stroke-width:2"/><path d="M234,147 l-9,-4 l0,8 z" style="fill:var(--line)"/>
    <rect x="236" y="104" width="100" height="86" rx="9" style="fill:var(--accent-soft);stroke:var(--accent);stroke-width:1.5"/><text x="286" y="140" text-anchor="middle" style="fill:var(--accent-ink);font-weight:700">② TimeTick</text><text x="286" y="162" text-anchor="middle" style="fill:var(--muted)">stamp ts</text>
    <line x1="336" y1="147" x2="352" y2="147" style="stroke:var(--line);stroke-width:2"/><path d="M352,147 l-9,-4 l0,8 z" style="fill:var(--line)"/>
    <rect x="354" y="104" width="100" height="86" rx="9" style="fill:var(--panel);stroke:var(--purple);stroke-width:1.5"/><text x="404" y="140" text-anchor="middle" style="fill:var(--purple);font-weight:700">③ Txn</text><text x="404" y="162" text-anchor="middle" style="fill:var(--muted)">atomic</text>
    <line x1="454" y1="147" x2="470" y2="147" style="stroke:var(--line);stroke-width:2"/><path d="M470,147 l-9,-4 l0,8 z" style="fill:var(--line)"/>
    <rect x="472" y="104" width="100" height="86" rx="9" style="fill:var(--panel);stroke:var(--teal);stroke-width:1.5"/><text x="522" y="140" text-anchor="middle" style="fill:var(--teal);font-weight:700">④ Shard</text><text x="522" y="162" text-anchor="middle" style="fill:var(--muted)">assign seg</text>
    <line x1="572" y1="147" x2="592" y2="147" style="stroke:var(--accent);stroke-width:2.5"/><path d="M592,147 l-11,-5 l0,10 z" style="fill:var(--accent)"/>
  ...
  <div class="figcap"><b>The interceptor chain = an assembly line</b>: every <span class="mono">Append</span> threads <b>① Lock</b> (outermost — acquired first and held across the whole chain) <b>② TimeTick</b> (stamp a PChannel-level monotonic ts — what the read path's tsafe waits on) <b>③ Txn</b> (transaction atomicity, handled inside the TimeTick interceptor) <b>④ Shard</b> (segment assignment — the segment ID is fixed here), then <b>⑤</b> truly lands in the WAL Backend. Concerns stay orthogonal and pluggable — a new write semantic is just one more link, no change to the core append.</div>
</div>

<div class="vflow">
  <div class="step"><div class="num">1</div><div class="sc"><h4>Lock interceptor (outermost)</h4><p><strong>acquires the lock first and holds it</strong> across the whole chain — <strong>exclusive/shared</strong> access control at VChannel or PChannel scope, ensuring correct ordering of concurrent writes. It sits at the <strong>outer layer</strong>.</p></div></div>
  <div class="step"><div class="num">2</div><div class="sc"><h4>TimeTick interceptor</h4><p>stamps the message with a PChannel-level <strong>monotonically increasing</strong> TimeTick — the "log clock" that the read path's tsafe waits on (Lesson 30).</p></div></div>
  <div class="step"><div class="num">3</div><div class="sc"><h4>Txn (handled inside the TimeTick interceptor)</h4><p>manages the <strong>transaction</strong> lifecycle: framing a batch that should be "all-or-nothing visible" into one atomic log transaction.</p></div></div>
  <div class="step"><div class="num">4</div><div class="sc"><h4>Shard interceptor</h4><p>does <strong>segment assignment</strong>: which growing segment these rows land in (the segment ID is decided here — recall Lesson 15, "segment ID is assigned at the StreamingNode").</p></div></div>
...
<div class="card key">
  ...
  <ul>
    ...
    <li><strong>Interceptor chain</strong>: every Append passes <strong>Lock / TimeTick / Txn / Shard</strong> in order (Lock is outermost, acquired and held first), separating locking, timestamp-stamping, transactions and segment assignment.</li>
...
<div class="cols">
  <div class="col"><h4>① 异地容灾 DR</h4><p>异地维持一个热备、时刻跟随主集群；主集群机房整体故障时，把热备<strong>提升为主</strong>继续服务，数据几乎不丢。</p></div>
  <div class="col"><h4>② 跨地域只读</h4><p>离用户近的备集群承接<strong>本地读</strong>，降低跨域延迟；写仍走主集群，读就近分担。</p></div>
  <div class="col"><h4>③ 平滑迁移</h4><p>搬到新集群/新版本时，让新集群作备<strong>追平</strong>全部历史与增量，追平后一次切换，<strong>几乎无停机</strong>。</p></div>
</div>
...
<div class="cols">
  ...
  <div class="col"><h4>② Cross-region reads</h4><p>a standby near users serves <strong>local reads</strong>, cutting cross-region latency; writes still go to the primary, reads shared locally.</p></div>
  <div class="col"><h4>③ Smooth migration</h4><p>moving to a new cluster/version, let the new one stand by and <strong>catch up</strong> all history + increment, then switch once — <strong>near-zero downtime</strong>.</p></div>
</div>

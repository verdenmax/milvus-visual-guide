...
<p>这些索引并非 Milvus 自研，而是来自它的向量检索<strong>内核库 Knowhere</strong>（一个上游库）。在 Milvus 侧的 C++ 代码里，能看到它<strong>引用</strong>这些 Knowhere 索引枚举常量。
下面这段就是 Milvus 内核里列举索引类型的地方（注意：Knowhere 本身是独立仓库，这里只展示 Milvus 引用它的位置，<strong>不杜撰 Knowhere 的文件路径</strong>）：</p>

<div class="codefile">
  <div class="cf-head"><span class="dot"></span><span class="path">internal/core/src/index/Utils.cpp</span><span class="ln">引用 knowhere::IndexEnum</span></div>
  <pre><span class="cm">// Milvus 内核侧引用 Knowhere 提供的索引类型常量</span>
knowhere::IndexEnum::INDEX_FAISS_IVFFLAT      <span class="cm">// IVF_FLAT</span>
knowhere::IndexEnum::INDEX_FAISS_BIN_IVFFLAT  <span class="cm">// 二值向量的 IVF</span>
knowhere::IndexEnum::INDEX_HNSW               <span class="cm">// HNSW 图索引</span>
knowhere::IndexEnum::INDEX_DISKANN            <span class="cm">// DiskANN（磁盘驻留）</span>
knowhere::IndexEnum::INDEX_SPARSE_INVERTED_INDEX <span class="cm">// 稀疏向量倒排</span></pre>
</div>
...
<div class="card key">
  ...
  <ul>
    ...
    <li>这些是 <strong>Knowhere</strong> 索引类型（Milvus 引用，见 <span class="mono">index/Utils.cpp</span>）；构建细节留到第五部分。</li>
  </ul>
</div>
""",
    "en": r"""
...
<p>These indexes aren't built in-house by Milvus; they come from its vector-search <strong>kernel library Knowhere</strong> (an upstream lib).
On the Milvus side, the C++ code <strong>references</strong> these Knowhere index enum constants. Below is where the Milvus kernel enumerates
index types (note: Knowhere is its own repository — we only show where Milvus references it, <strong>without fabricating Knowhere's file
paths</strong>):</p>

<div class="codefile">
  <div class="cf-head"><span class="dot"></span><span class="path">internal/core/src/index/Utils.cpp</span><span class="ln">references knowhere::IndexEnum</span></div>
  <pre><span class="cm">// Milvus kernel side references the index-type constants Knowhere provides</span>
knowhere::IndexEnum::INDEX_FAISS_IVFFLAT      <span class="cm">// IVF_FLAT</span>
knowhere::IndexEnum::INDEX_FAISS_BIN_IVFFLAT  <span class="cm">// IVF for binary vectors</span>
knowhere::IndexEnum::INDEX_HNSW               <span class="cm">// HNSW graph index</span>
knowhere::IndexEnum::INDEX_DISKANN            <span class="cm">// DiskANN (disk-resident)</span>
knowhere::IndexEnum::INDEX_SPARSE_INVERTED_INDEX <span class="cm">// sparse-vector inverted index</span></pre>
</div>
...
<div class="card key">
  ...
  <ul>
    ...
    <li>These are <strong>Knowhere</strong> index types (referenced by Milvus, see <span class="mono">index/Utils.cpp</span>); build details are saved for Part 5.</li>
  </ul>
</div>
""",
}
...
<div class="codefile">
  <div class="cf-head"><span class="dot"></span><span class="path">SegmentState 枚举（commonpb，被 datacoord 引用）</span><span class="ln">internal/datacoord 中的 commonpb.SegmentState_*</span></div>
  <pre><span class="cm">// 段的生命周期状态（Milvus 代码逐字定义，datacoord 大量引用）</span>
...
<div class="codefile">
  <div class="cf-head"><span class="dot"></span><span class="path">SegmentState enum (commonpb, referenced by datacoord)</span><span class="ln">commonpb.SegmentState_* in internal/datacoord</span></div>
  <pre><span class="cm">// Segment lifecycle states (literal Milvus code, heavily used by datacoord)</span>

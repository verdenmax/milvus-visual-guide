...
<p>You might ask: if it's peak performance you're after, why not Rust? It's a fair question, and the answer is <strong>history and ecosystem</strong>. When Milvus got started, the mature kernel libraries for vector search — especially the <strong>Knowhere/FAISS</strong> lineage — along with the various GPU compute toolchains were almost entirely a <strong>C++</strong> world. Writing the core in C++ lets Milvus <strong>reuse those high-performance building blocks directly</strong> instead of reinventing them. Milvus did later bring in <strong>a little Rust</strong> (Lesson 24's tantivy full-text index is written in Rust and wrapped for C++ to call), which shows it isn't hostile to more modern languages — <strong>what gets used where simply depends on which ecosystem is most mature and which migration is cheapest</strong>. It's a pragmatic stance: <strong>a language is a tool, not a faith</strong>. Once you see that, "why C++" stops being a puzzle and becomes an appreciation of Milvus picking the fittest tool at each layer.</p>

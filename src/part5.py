...
<div class="fig">
  <svg viewBox="0 0 760 300" role="img" aria-label="索引引擎版本协商：滚动升级期新老 QueryNode 并存，DataCoord 让 worker 按最低兼容版本建索引，最老的节点也能加载">
    ...
    <text x="128" y="110" text-anchor="middle" class="mono" style="fill:var(--muted)">current = v3</text>
    <text x="128" y="132" text-anchor="middle" class="mono" style="fill:var(--teal)">min 兼容 = v2</text>
    ...
    <rect x="262" y="82" width="150" height="46" rx="9" style="fill:var(--accent-soft);stroke:var(--accent)"/><text x="337" y="102" text-anchor="middle" style="fill:var(--accent-ink);font-weight:700">worker 建索引</text><text x="337" y="120" text-anchor="middle" style="fill:var(--muted)">按 min = v2 产出</text>
    ...
    <rect x="588" y="136" width="152" height="38" rx="7" style="fill:var(--panel);stroke:var(--teal);stroke-width:1.5"/><text x="664" y="160" text-anchor="middle" style="fill:var(--ink)">QueryNode v2（老）✓</text>
    <text x="385" y="212" text-anchor="middle" style="fill:var(--red);font-weight:700">✗ 若按最新 v3 建 → v2 老节点读不懂、加载失败</text>
    <text x="385" y="244" text-anchor="middle" style="fill:var(--muted)">∴ 产出格式向最保守（最老）的消费者看齐 —— 滚动升级期服务不中断</text>
  </svg>
  <div class="figcap"><b>建索引，向最弱的消费者看齐</b>：升级是<b>滚动</b>的，新老 QueryNode 总会并存一段时间。DataCoord 的版本管理器跟踪各节点版本，给出<b>最低兼容版本</b>；worker 就<b>按 min 版本建索引</b>，于是连最老的节点也能加载。这正是"<b>兼容性优先于新特性</b>"——升级期始终不丢服务。</div>
</div>
...
<div class="fig">
  <svg viewBox="0 0 760 300" role="img" aria-label="Index engine version negotiation: during a rolling upgrade old and new QueryNodes coexist; DataCoord has the worker build at the minimal compatible version so even the oldest node can load it">
    <rect x="30" y="58" width="196" height="92" rx="11" style="fill:var(--panel);stroke:var(--accent);stroke-width:1.5"/>
    <text x="128" y="84" text-anchor="middle" style="fill:var(--accent-ink);font-weight:700">DataCoord version mgr</text>
    <text x="128" y="110" text-anchor="middle" class="mono" style="fill:var(--muted)">current = v3</text>
    <text x="128" y="132" text-anchor="middle" class="mono" style="fill:var(--teal)">min compat = v2</text>
    <line x1="226" y1="104" x2="260" y2="104" style="stroke:var(--accent);stroke-width:2"/><path d="M260,104 l-11,-5 l0,10 z" style="fill:var(--accent)"/>
    <rect x="262" y="82" width="150" height="46" rx="9" style="fill:var(--accent-soft);stroke:var(--accent)"/><text x="337" y="102" text-anchor="middle" style="fill:var(--accent-ink);font-weight:700">worker builds</text><text x="337" y="120" text-anchor="middle" style="fill:var(--muted)">at min = v2</text>
    <line x1="412" y1="104" x2="446" y2="104" style="stroke:var(--teal);stroke-width:2"/><path d="M446,104 l-11,-5 l0,10 z" style="fill:var(--teal)"/>
    <rect x="448" y="84" width="96" height="42" rx="8" style="fill:var(--teal-soft);stroke:var(--teal)"/><text x="496" y="110" text-anchor="middle" class="mono" style="fill:var(--teal);font-weight:700">index v2</text>
    <line x1="544" y1="98" x2="586" y2="66" style="stroke:var(--teal);stroke-width:1.5"/><path d="M586,66 l-11,1 l4,8 z" style="fill:var(--teal)"/>
    <line x1="544" y1="105" x2="586" y2="109" style="stroke:var(--teal);stroke-width:1.5"/><path d="M586,109 l-11,-2 l1,8 z" style="fill:var(--teal)"/>
    ...
    <rect x="588" y="48" width="152" height="38" rx="7" style="fill:var(--panel);stroke:var(--teal)"/><text x="664" y="72" text-anchor="middle" style="fill:var(--ink)">QueryNode v3 ✓</text>
    <rect x="588" y="92" width="152" height="38" rx="7" style="fill:var(--panel);stroke:var(--teal)"/><text x="664" y="116" text-anchor="middle" style="fill:var(--ink)">QueryNode v3 ✓</text>
    <rect x="588" y="136" width="152" height="38" rx="7" style="fill:var(--panel);stroke:var(--teal);stroke-width:1.5"/><text x="664" y="160" text-anchor="middle" style="fill:var(--ink)">QueryNode v2 (old) ✓</text>
    <text x="385" y="212" text-anchor="middle" style="fill:var(--red);font-weight:700">✗ if built at the newest v3 → the v2 old node can't read it, load fails</text>
    <text x="385" y="244" text-anchor="middle" style="fill:var(--muted)">∴ align the output format with the most conservative (oldest) consumer —— no service lost while upgrading</text>
  </svg>
  <div class="figcap"><b>Build to the weakest consumer</b>: upgrades are <b>rolling</b>, so old and new QueryNodes always coexist for a while. DataCoord's version manager tracks each node's version and yields the <b>minimal compatible version</b>; the worker <b>builds at that min version</b>, so even the oldest node can load it. That is "<b>compatibility over new features</b>" — no service lost during the upgrade.</div>
</div>

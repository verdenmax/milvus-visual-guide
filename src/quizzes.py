...
QUIZZES = {
    ...
    "21-index-service.html": {
        "mcq": [
            ...
            {
                "q": {
                    "zh": "如果某个段的索引构建任务执行到一半，worker 崩溃了，会发生什么？",
                    "en": "If a segment's index build task crashes halfway through on the worker, what happens?",
                },
                "opts": [
                    {
                        "zh": "DataCoord 的 index_inspector 会周期性巡检：发现该段仍缺索引，就重新派发构建任务；任务以段为单位、可重入，崩溃只是重来一次，不影响其他已建好的段",
                        "en": "DataCoord's index_inspector periodically reconciles: seeing the segment still lacks an index, it re-dispatches the build task; tasks are per-segment and idempotent, so a crash just retries without affecting other already-built segments",
                    },
                    {"zh": "整个集合的索引全部作废，必须从头重建", "en": "The whole collection's index is invalidated and must be rebuilt from scratch"},
                    {"zh": "该段从此永远没有索引，只能暴力扫描", "en": "That segment stays index-less forever and can only be brute-forced"},
                    {"zh": "Proxy 会阻塞所有写入直到该段构建成功", "en": "The Proxy blocks all writes until that segment's build succeeds"},
                ],
                "answer": 0,
                "why": {
                    "zh": "“按段派发 + 周期巡检 + 可重入任务”让索引构建天然容错：失败的段会被重新调度，成功的段不受牵连。这是把大任务拆成独立小任务、再用对账（reconcile）兜底的典型分布式设计",
                    "en": "'Per-segment dispatch + periodic reconcile + idempotent tasks' makes index build fault-tolerant by design: a failed segment gets rescheduled while successful ones are untouched. It's the classic distributed pattern of splitting a big job into independent small tasks backstopped by reconciliation",
                },
            },
        ],
        "open": [
    ...
    "22-knowhere.html": {
        "mcq": [
            ...
            {
                "q": {
                    "zh": "Knowhere 与 Faiss、HNSWlib、DiskANN、cuVS 这些库是什么关系？",
                    "en": "What is the relationship between Knowhere and libraries like Faiss, HNSWlib, DiskANN, and cuVS?",
                },
                "opts": [
                    {
                        "zh": "Knowhere 是一个 C++ 的统一抽象层，把这些第三方 ANN 库封装在同一套接口（建索引/序列化/搜索）之下；Milvus 只面向 Knowhere 编程，新增一种索引算法通常只需在 Knowhere 接入，不必改动 milvus core",
                        "en": "Knowhere is a C++ unified abstraction layer that wraps these third-party ANN libraries behind one interface (build/serialize/search); Milvus programs against Knowhere only, so adding a new index algorithm usually just means integrating it in Knowhere, not changing milvus core",
                    },
                    {"zh": "Knowhere 是 Faiss 的一个 Python 绑定", "en": "Knowhere is a Python binding for Faiss"},
                    {"zh": "Knowhere 负责对象存储，与索引算法无关", "en": "Knowhere handles object storage and has nothing to do with index algorithms"},
                    {"zh": "这些库各自是独立进程，Knowhere 只做 RPC 转发", "en": "Each library is a separate process and Knowhere only does RPC forwarding"},
                ],
                "answer": 0,
                "why": {
                    "zh": "把多种 ANN 库统一到一层接口背后，Milvus 就获得了“算法可插拔”：升级或新增索引主要在 Knowhere 完成，core 保持稳定。这正是把专门、易变的算法部分与稳定的系统骨架解耦",
                    "en": "By unifying many ANN libraries behind one interface, Milvus gains 'pluggable algorithms': upgrading or adding indexes happens mostly in Knowhere while core stays stable. This decouples the specialized, fast-moving algorithm part from the stable system skeleton",
                },
            },
        ],
        "open": [
    ...
    "23-index-build-and-load.html": {
        "mcq": [
            ...
            {
                "q": {
                    "zh": "QueryNode 用 mmap 方式加载索引/数据，主要换来了什么、又付出什么代价？",
                    "en": "When a QueryNode loads index/data via mmap, what does it mainly gain and at what cost?",
                },
                "opts": [
                    {
                        "zh": "好处是不必把整个索引读进物理内存——靠操作系统按页惰性加载，于是“大于内存”的段也能服务，单机能承载的数据量大增；代价是冷页首次访问要触发缺页、读磁盘，尾延迟可能升高（内存 vs 延迟的取舍）",
                        "en": "The gain is not having to read the whole index into physical RAM—the OS lazily pages it in on demand, so segments 'larger than memory' can still be served and a node holds far more data; the cost is that a cold page's first access triggers a page fault and disk read, so tail latency may rise (a memory-vs-latency tradeoff)",
                    },
                    {"zh": "既省内存又一定更快，没有任何代价", "en": "It saves memory and is always faster, with no cost at all"},
                    {"zh": "mmap 会把数据复制三份以保证可靠", "en": "mmap makes three copies of the data for reliability"},
                    {"zh": "mmap 只能用于 growing 段", "en": "mmap can only be used for growing segments"},
                ],
                "answer": 0,
                "why": {
                    "zh": "mmap 把“放不放得下”从硬约束变成“放得下多少在内存”的弹性问题：热点页常驻、冷页换出，用可能的缺页延迟换取超出物理内存的容量。是否启用、对哪些字段启用，是典型的容量/延迟权衡（详见第 35 课 mmap）",
                    "en": "mmap turns 'does it fit?' from a hard limit into an elastic 'how much stays in RAM': hot pages resident, cold pages evicted, trading possible page-fault latency for capacity beyond physical memory. Whether to enable it, and for which fields, is a classic capacity/latency tradeoff (see Lesson 35 mmap)",
                },
            },
        ],
        "open": [
    ...
    "24-scalar-and-fulltext.html": {
        "mcq": [
            ...
            {
                "q": {
                    "zh": "Milvus 的全文检索（BM25）大致是怎么工作的？",
                    "en": "Roughly how does Milvus's full-text search (BM25) work?",
                },
                "opts": [
                    {
                        "zh": "文本被分词后建成倒排索引（由 Rust 的 tantivy 引擎落地），查询时按 BM25 相关性打分排序；在 Milvus 里 BM25 的结果以稀疏向量的形式表达，从而能与向量检索统一在同一套 search 框架里",
                        "en": "Text is tokenized into an inverted index (delivered by Rust's tantivy engine), and queries are scored/ranked by BM25 relevance; in Milvus the BM25 result is expressed as sparse vectors, so it unifies with vector search under one search framework",
                    },
                    {"zh": "全文检索就是对原始字符串做 LIKE 全表扫描", "en": "Full-text search is just a LIKE full-table scan over raw strings"},
                    {"zh": "BM25 由 Knowhere 的 HNSW 索引实现", "en": "BM25 is implemented by Knowhere's HNSW index"},
                    {"zh": "全文检索必须先把文本转成稠密 embedding 才能查", "en": "Full-text search must first convert text into dense embeddings before querying"},
                ],
                "answer": 0,
                "why": {
                    "zh": "全文检索走的是“分词→倒排→BM25 打分”这条经典信息检索路径，由 tantivy（Rust）落地，不属于 Knowhere。Milvus 把 BM25 输出表达成稀疏向量，于是稀疏（关键词）与稠密（语义）能在同一套检索/混合排序框架里协同（详见第 48 课）",
                    "en": "Full-text search follows the classic IR path 'tokenize → inverted index → BM25 scoring,' delivered by tantivy (Rust), not Knowhere. Milvus expresses BM25 output as sparse vectors, so sparse (keyword) and dense (semantic) cooperate under one search/hybrid-ranking framework (see Lesson 48)",
                },
            },
        ],
        "open": [
